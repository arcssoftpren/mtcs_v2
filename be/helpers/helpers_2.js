...
class leaderInspection {
  ...
  init(tool, date) {
    this.date = date;
    this.toolId = tool.id;
    this.inspectionItems = JSON.parse(tool.inspectionItems);
    this.equipmentName = JSON.parse(tool.headerData).equipmentName;
    this.regisNo = JSON.parse(tool.headerData).regisNo;
    ...
    this.dates = weeksDate.find((week) =>
    ...
    this.id = `${this.toolId}-${this.year}${this.month}${index}`;
  }
}
